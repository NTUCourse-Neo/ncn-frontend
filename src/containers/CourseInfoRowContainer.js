            ...
            props.courseInfo.map((info, index) => {
                return(
                    <Accordion allowToggle w="100%" key={index} onMouseEnter={() => props.setHoveredCourse(info)} onMouseLeave={() => {props.setHoveredCourse(null)}}>
                        <CourseInfoRow id={info["id"]} index={index} courseInfo={info} selected={props.selectedCourses.includes(info._id)} setHoveredCourse={props.setHoveredCourse} isfavorite={userInfo.db.favorites.includes(info._id)}/>
                        <Spacer my="1" />
                    </Accordion>
                );
            }
