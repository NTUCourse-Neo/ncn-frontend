...
import { FaAngleLeft, FaChevronDown, FaChevronUp, FaPlus, FaMinus, FaArrowRight, FaRss, FaCalendar, FaRegCalendarAlt } from 'react-icons/fa';
  ...
  const renderSettingSwitch = (label, default_checked, isDisabled) => {
    ...
    const renderCourseTable = () => {
      if(isMobile){
        return(
          <>
            <Modal isOpen={displayTable} onClose={()=>{setDisplayTable(false)}} size="full">
              <ModalOverlay />
              <ModalContent>
        ...
        <>
          {renderNoLoginWarning()}
            ...
            <Fade in={!displayTable}>
                <Flex as="button" flexDirection={isMobile? "row":"column"} alignItems="center" justifyContent="center" position="absolute" top={isMobile? "85vh":"80vh"} left={isMobile? "70vw":"90vw"} bg="gray.50" boxShadow="md" py={isMobile?"1":"4"} px={isMobile? "4":"2"} borderRadius="xl"
                onClick={() => setDisplayTable(!displayTable)} _hover={{boxShadow:"lg", transform:"translateY(-2px) scale(1.02)"}} transition="all 200ms">
                  <Icon as={FaRegCalendarAlt} mr={isMobile ? "2":"0"} boxSize="4" color="teal.500" />
                  <Text my="2" fontWeight={800} fontSize={isMobile? "md":"lg"} color="teal.500" style={isMobile?{}:{writingMode: "vertical-rl"}}>課表</Text>
                </Flex>
            </Fade>
            { renderCourseTable() }
        </>
