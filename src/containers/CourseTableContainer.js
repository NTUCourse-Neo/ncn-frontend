...
function CourseTableContainer(props) {
  ...
  const renderCourseBox = (course, hover, day, interval, grow) => {
    if (course.time_map[day][interval].includes(hover.time_map[day][interval][0])){
      return(
        <CourseTableCard grow={grow} hoverId={hover.time_map[day][interval][0]} courseTime={course.time_map[day][interval]} courseData={props.courses} interval={interval} day={weekdays_map[day]}/>
      );
    }
    return(
      <>
        <CourseTableCard grow={grow} isHover courseTime={course.time_map[day][interval]} courseData={hover.course_data} interval={interval} day={weekdays_map[day]}/>
        <CourseTableCard grow={grow} courseTime={course.time_map[day][interval]} courseData={props.courses} interval={interval} day={weekdays_map[day]}/>
      </>
    );
  };
          ...
          days.map((day, j) => {
            ...
            if (props.courseTimes.time_map && day in props.courseTimes.time_map && interval in props.courseTimes.time_map[day]){
              if(props.hoveredCourse && props.hoveredCourseTime && day in props.hoveredCourseTime.time_map && interval in props.hoveredCourseTime.time_map[day]){
                return(
                  <Td>
                    {renderCourseBox(props.courseTimes, props.hoveredCourseTime, day, interval, grow)}
                  </Td>
                );
              }
              return(
                <Td key={i+"_"+j}>
                  <CourseTableCard grow={grow} courseTime={props.courseTimes.time_map[day][interval]} courseData={props.courses} interval={interval} day={weekdays_map[day]}/>
                </Td>
              );
            }
            if(props.hoveredCourse && props.hoveredCourseTime && day in props.hoveredCourseTime.time_map && interval in props.hoveredCourseTime.time_map[day]){
              return(
                <Td key={i+"_"+j}>
                  <CourseTableCard grow={grow} isHover courseTime={[]} courseData={props.hoveredCourseTime.course_data} interval={interval} day={weekdays_map[day]}/>
                </Td>
              );
            }
            return(
              <Td key={i+"_"+j}><Flex w="100%" h="3vh" mb="1" justifyContent="center" alignItems="center">
