...
function CourseDeadlineCountdown() {
  ...
  const elasped_days =
    status_idx === 0
      ? Math.floor((course_select_schedule[schedule_idx].start - ts) / 86400)
      : Math.floor((course_select_schedule[schedule_idx].end - ts) / 86400);
  const elapsed_hours =
    status_idx === 0
      ? Math.floor((course_select_schedule[schedule_idx].start - ts) / 3600) % 24
      : Math.floor((course_select_schedule[schedule_idx].end - ts) / 3600) % 24;
  const time_percent =
    status_idx === 0
      ? 0
      : (ts - course_select_schedule[schedule_idx].start) / (course_select_schedule[schedule_idx].end - course_select_schedule[schedule_idx].start);
      ...
      >
        <Flex w="100%" justifyContent="space-between" alignItems="center" wrap="wrap" css={{ gap: "2" }}>
          <Text fontSize="xl" fontWeight="800" color="gray.700" mb="2">
            {status_map[status_idx].emoji} {course_select_schedule[schedule_idx].name} {status_map[status_idx].name}
          </Text>
          <HStack ml="1">
            <FaClock />
            <Text fontSize="sm" fontWeight="600" color="gray.700" mb="2">
              {" "}
              尚餘 {elasped_days} 天 {elapsed_hours} 時
            </Text>
          </HStack>
        </Flex>
        ...
        <Flex w="100%" justifyContent="space-between" alignItems="center">
          {course_select_schedule.map((item, idx) => {
            return (
              <Text key={idx} fontSize="xs" fontWeight="600" color="gray.700" mb="2">
                {item.label}
              </Text>
            );
          })}
