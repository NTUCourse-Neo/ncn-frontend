  ...
  const renderSignInButton = () => {
    if (isMobile) {
      ...
      if (isAuthenticated) {
        return (
          <>
            <Menu>
              <MenuButton as={Avatar} name={user.name} src={user.picture} _hover={{ cursor: "pointer" }}></MenuButton>
              <MenuList>
                <MenuGroup title="帳戶">
                  <Link to="/user/info">
      ...
      return (
        <>
          <Menu>
            <MenuButton as={Avatar} _hover={{ cursor: "pointer" }}></MenuButton>
            <MenuList>
              <MenuGroup title="更多">
                <Link to="/about">
                  <MenuItem>關於</MenuItem>
    ...
    if (isAuthenticated) {
      return (
        <>
          <Link to="/user/my">
            <Button colorScheme="blue" variant="ghost" size="md" mx="2" mr="4" leftIcon={<FaBookmark />}>
              最愛
            </Button>
          </Link>
          <Menu>
    ...
    return (
      <>
        <Link to="/about">
          <Button colorScheme="blue" variant="ghost" leftIcon={<FaInfoCircle />} size="md" ml="10px" mr="10px">
            關於
          </Button>
        </Link>
        <Button colorScheme="yellow" rightIcon={<ChevronRightIcon />} size="md" ml="10px" mr="10px" onClick={() => loginWithRedirect()}>
          登入 / 註冊
        </Button>
      </>
    );
  ...
  if (isMobile) {
    return (
      <>
        ...
        >
          <Link to="/">
            <Heading fontSize="xl" fontWeight="700" mr="auto" color="gray.600" minW="200px">
              NTUCourse Neo
            </Heading>
          </Link>
          <Link to="/course">
            <Button colorScheme="blue" variant="ghost" size="md" leftIcon={<FaBook />}>
              課程
            </Button>
          </Link>
          {renderSignInButton()}
  ...
  return (
    <Flex position="fixed" w="100%" h="64px" bg="teal.300" flexDirection="row" justifyContent="start" alignItems="center" zIndex="1000">
      <Flex justifyContent="center" alignItems="center" ml="60px">
        <Link to="/">
          <Heading fontSize="2xl" fontWeight="700" mr="auto" color="gray.700" minW="200px">
            NTUCourse Neo
          </Heading>
        </Link>
        <Link to="/course">
          <Button colorScheme="blue" variant="ghost" size="md" ml="30px" leftIcon={<FaBook />}>
            課程
          </Button>
        </Link>
      </Flex>
